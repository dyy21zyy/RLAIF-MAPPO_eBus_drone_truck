...
from data_pipeline.generate_station_base_load import generate_station_base_load
...
def build_instance(config_path: str | Path, fallback: bool = False, output_root: str | Path | None = None, custom_bus_route: str | Path | None = None, seed_overrides: ScenarioSeedTuple | dict[str, int] | None = None) -> dict[str, Any]:
    ...
    station_load_paths = generate_station_base_load(stations, config, output_dir)
    ...
    artifact_names = {**{key: path.name for key, path in road_paths.items()},
                      ...
                      **{key: path.name for key, path in station_load_paths.items()},
