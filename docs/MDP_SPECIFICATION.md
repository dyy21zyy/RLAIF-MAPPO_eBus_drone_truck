...
## Fix Phase 2 stop-by-stop physical-bus event chain

Bus service now uses automatic `bus_trip_start`, `bus_arrive_stop`, `bus_depart_stop`, `bus_trip_complete`, and `bus_relocation_complete` events. Only `BUS_TERMINAL_DEPARTURE` and `BUS_STATION_ARRIVAL` are MAPPO-facing bus decisions. Event ordering is by event time, explicit priority, and deterministic sequence ID; for bus events, segment arrivals are processed before decision/departure propagation, and downstream arrivals are generated only after actual departures are known.
