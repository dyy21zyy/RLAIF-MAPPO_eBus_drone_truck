...
def build_plan(*, output_root: Path, commit: str, train_manifest: Path | None = None, device: str = "cuda") -> list[PhaseSpec]:
    ...
    audit=pref_root/"legacy_audit"
    legacy_path=os.environ.get("FORMAL_LEGACY_ASSIGNMENT_PREFERENCES", "MISSING_FORMAL_LEGACY_ASSIGNMENT_PREFERENCES")
    legacy_sha=os.environ.get("FORMAL_LEGACY_ASSIGNMENT_PREFERENCES_SHA256", "MISSING_FORMAL_LEGACY_ASSIGNMENT_PREFERENCES_SHA256")
    ...
    return [
      ...
      PhaseSpec(2,"assignment_preferences_and_reward_model",((py,"-m","experiments.audit_assignment_preferences","--input",legacy_path,"--accepted-output",str(audit/"accepted_legacy_assignment.jsonl"),"--quarantine-output",str(audit/"quarantined_legacy_assignment.jsonl"),"--audit-json",str(audit/"legacy_preference_quality_audit.json"),"--audit-md",str(audit/"legacy_preference_quality_audit.md"),"--expected-count","200","--expected-sha256",legacy_sha),(py,"-m","experiments.generate_formal_multiagent_preferences","--config","configs/paper/rlaif_preference_generation.yaml","--output-root",str(pref_root),"--agents","assignment","--cache-dir",str(pref_root/"evaluator_cache_v2")),(py,"-m","experiments.train_multi_agent_reward_models","--preferences",str(pref_file),"--config",str(resolved/"train_reward_assignment.yaml"),"--agent","assignment","--output",str(reward),"--device",device)),(audit/"legacy_preference_quality_audit.json",pref_file,pref_root/"preference_manifest.json",reward),(Path("configs/paper/rlaif_preference_generation.yaml"),train,Path("configs/paper/train_reward_assignment.yaml")),"assignment_preference_quality_and_reward_v2"),
...
def _run_execute_phase(phase: PhaseSpec, context: ExperimentContext, digest: str) -> None:
    ...
    try:
        if phase.phase==0: materialize_initial_configs(root,context.device); gpu_readiness(root/"gpu_readiness.json",context.device,require_cuda=context.device != "cpu"); restore_phase_0_context(context)
            ...
            _run_command(phase.commands[0],phase,root,0)
            audit=json.loads((root/"preferences"/"legacy_audit"/"legacy_preference_quality_audit.json").read_text())
            if audit.get("external_api_call_count") != 0: raise ValueError("legacy audit must make zero evaluator API calls")
            _run_command(phase.commands[1],phase,root,1); context.preference_artifact=validate_assignment_preferences(root/"preferences")
            _run_command(phase.commands[2],phase,root,2); context.reward_model_artifact=validate_reward_checkpoint(root/"reward_models"/"reward_assignment.pt",context.preference_artifact)
        elif phase.phase==3:
