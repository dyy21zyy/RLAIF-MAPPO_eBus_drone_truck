...
def collect_decision_states(config: dict[str, Any]) -> list[dict[str,Any]]:
    manifest=Path(config.get('scenario_bank',{}).get('final_train_manifest',''))
    ...
    for p in data.get('decision_state_files',[]):
        records += read_jsonl(p)
    out=[s for r in records if (s:=normalize_decision_state(r))]
    if not out: raise RuntimeError('no decision states found in formal train bank manifest')
    return out
...
def generate(config_path:Path, output_root:Path, *, resume:bool=False, api_call:Callable[[str,APISettings],str]|None=None) -> dict[str,Any]:
    ...
    states=collect_decision_states(cfg)
    ...
    manifest={'status':'complete','agents':{},'split_hash':split['hash'],'scenario_bank_manifest':str(cfg['scenario_bank']['final_train_manifest']),'scenario_bank_hash':sha_file(Path(cfg['scenario_bank']['final_train_manifest']))}
